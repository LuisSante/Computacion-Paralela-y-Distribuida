...
void Odd_even(int a[], int n)
{
    ...
    {
        if (phase % 2 == 0) // pasos pares, intercambio con los del lado derecho
        // shared indica que a y n NO son variables locales
        // private indica que i y tmp son variables locales en subprocesos bifurcados


        // en cada iteracion crea y destruye los threads por ello es mas pesado
        // en cada iteracion se crearan el numero de "threads_count" y luego de destruirán 


        //La default(none) obliga a un programador a especificar explícitamente los atributos de intercambio de datos de todas las variables.
        /*BUENAS PRACTICAS DE PROGRAMACION*/
        //escribir siempre regiones paralelas con la default(none)
        //declarar variables privadas dentro de regiones paralelas siempre que sea posible 
#pragma omp parallel for num_threads(thread_count) default(none) shared(a, n) private(i, tmp)
        ...
        else // pasos impares, intercambio con los del pado izquierdo
#pragma omp parallel for num_threads(thread_count) default(none) shared(a, n) private(i, tmp)
            for (i = 1; i < n - 1; i += 2)
            {
                //intercambio
